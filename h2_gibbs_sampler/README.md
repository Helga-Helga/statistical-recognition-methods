...
## The problem:
- We have an image ![img](http://latex.codecogs.com/svg.latex?%5Cvec%7Bx%7D) of size ![img](http://latex.codecogs.com/svg.latex?m+%5Ctimes+n), where each pixel is white or black (0 or 1)
...
- ![img](http://latex.codecogs.com/svg.latex?k_i%5Eh) is a state of row i
	- if row i is shaded, than ![img](http://latex.codecogs.com/svg.latex?k_i%5Eh+%3D+1) (**filled_row[i] = 1**)
	- if row i is not shaded, than ![img](http://latex.codecogs.com/svg.latex?k_i%5Eh+%3D+0) (**filled_row[i] = 0**)
- Each column is shaded with 0.5 probability: each pixel in the column becomes black
- ![img](http://latex.codecogs.com/svg.latex?k_j%5Ev) is a state of column j
	- if column j is shaded, then ![img](http://latex.codecogs.com/svg.latex?k_j%5Ev+%3D+1) (**filled_column[j] = 1**)
	- if row i is not shaded, then ![img](http://latex.codecogs.com/svg.latex?k_j%5Ev+%3D+0) (**filled_column[j] = 0**)
- Each pixel ( i, j) inverts its color with probability that is equal to ![img](http://latex.codecogs.com/svg.latex?%5Cvarepsilon) (![img](http://latex.codecogs.com/svg.latex?%5Cvarepsilon is known)),
  so that we obtain a noised image.
- The task is to find the most probable set of shaded rows and shaded columns
  of initial image, when only noised image and ![img](http://latex.codecogs.com/svg.latex?%5Cvarepsilon) are known.
...
> In this program the initial image and ![img](http://latex.codecogs.com/svg.latex?%5Cvarepsilon) are generated randomly.
> In the output you can see the initial image, filled rows and columns (ground truth), the value of ![img](http://latex.codecogs.com/svg.latex?%5Cvarepsilon), noised image, recognized rows and columns that were filled and the output image.
...
That means that pixel changed color with ![img](http://latex.codecogs.com/svg.latex?%5Cvarepsilon) probability and didn't change color with $1 - \varepsilon$ probability.
